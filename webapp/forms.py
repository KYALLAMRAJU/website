    ...
    def clean(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError(
                "This email is already registered.If you forgot your password please go to forgot password page to reset your password"
            )
        pattern = r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{8,}$"
        password = total_cleaned_data["password"]
        if not re.search(pattern, password):
            raise forms.ValidationError(
                "Password must contain at least 8 characters and should contain at least one uppercase letter, one lowercase letter, one digit, and one special character:@$!%*?&#"
            )
...
class resetpasswordForm(forms.Form):
    ...
    def clean(self):
        ...
        if email and not User.objects.filter(email=email).exists():
            raise forms.ValidationError(
                "This email is not registered. Please enter a valid registered email or signup one if you don't have an account."
            )
        ...
        if newpassword != confirmpassword:
            raise forms.ValidationError(
                "Password and Confirm Password do not match please enter same password in both fields"
            )
        bothandler = total_cleaned_data["bothandler"]
        if len(bothandler) > 0:
            raise forms.ValidationError("Dont play with my website")
...
class loginForm(forms.ModelForm):
    ...
    loginemail = forms.CharField(
        label="",
        widget=forms.EmailInput(
            attrs={"class": "form-control form-control-sm mb-3", "placeholder": "Enter your email"}
        ),
    )
